...
@dataclass
class StatsMergingConfig:
    """Configuration for StatsMerging.

    Attributes:
        mode: "task_wise" or "layer_wise"
        svd_rank: Top-r singular values used in stats feature vector
        hidden_dim: Hidden dimension of StatsMergeLearner MLP
        num_layers: Number of layers in StatsMergeLearner MLP
        lr: Learning rate for StatsMergeLearner
        epochs: Training epochs for StatsMergeLearner
        batch_size: Batch size for StatsMergeLearner training
        num_examples: Examples per locale for teacher distillation
        normalize: "softmax" or "none"
        dataset_split: Dataset split for distillation
    ...
    def __post_init__(self):
        if self.mode not in ("task_wise", "layer_wise"):
            raise ValueError(f"statsmerging.mode must be 'task_wise' or 'layer_wise', got '{self.mode}'")
        if self.svd_rank < 1:
            raise ValueError(f"statsmerging.svd_rank must be >= 1, got {self.svd_rank}")
        if self.hidden_dim < 1:
            raise ValueError(f"statsmerging.hidden_dim must be >= 1, got {self.hidden_dim}")
        if self.num_layers < 1:
            raise ValueError(f"statsmerging.num_layers must be >= 1, got {self.num_layers}")
        if self.epochs < 1:
            raise ValueError(f"statsmerging.epochs must be >= 1, got {self.epochs}")
        if self.batch_size < 1:
            raise ValueError(f"statsmerging.batch_size must be >= 1, got {self.batch_size}")
        if self.num_examples < 1:
            raise ValueError(f"statsmerging.num_examples must be >= 1, got {self.num_examples}")
        if self.normalize not in ("softmax", "none"):
            raise ValueError(f"statsmerging.normalize must be 'softmax' or 'none', got {self.normalize}")
