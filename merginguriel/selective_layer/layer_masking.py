...
def generate_layer_include_only_regex(include_layers: List[int]) -> List[str]:
    """
    Generate exclude regex for all layers NOT in include_layers.

    This is the inverse of generate_layer_exclude_regex: specify which layers
    to KEEP merged, and everything else gets excluded (retains base model values).
    ...
    Returns:
        List of regex patterns for layers to EXCLUDE from merging.

    Example:
        >>> generate_layer_include_only_regex([4, 5, 6, 7])
        ['roberta\\.encoder\\.layer\\.0\\.', ..., 'roberta\\.encoder\\.layer\\.11\\.']
        # Excludes layers 0-3 and 8-11, includes only 4-7
    """
    include_set = set(include_layers)
    exclude_layers = [i for i in range(NUM_LAYERS) if i not in include_set]
    return generate_layer_exclude_regex(exclude_layers)


def get_include_only_ablation_points() -> Dict[str, List[int]]:
    """
    Generate ablation points for include-only layer merging.

    Each entry maps an ablation name to the layers to INCLUDE (merge).
    All other layers retain pretrained base model weights.

    Returns:
        Dict mapping ablation name to layers to INCLUDE.

    Structure:
        - Baselines: include_all (full merge) and include_none (pretrained-only)
        - Single groups: bottom, middle, top (LinguaMap three-phase)
        - Two-group combinations: bottom+middle, middle+top, bottom+top
    """
    all_layers = list(range(NUM_LAYERS))
    points = {}

    # Baselines
    points["include_all_layers"] = all_layers.copy()
    points["include_no_layers"] = []

    # Single layer groups (LinguaMap phases)
    for group_name, group_layers in LAYER_GROUPS.items():
        points[f"include_only_group_{group_name}"] = group_layers.copy()

    # Two-group combinations
    points["include_only_bottom_middle"] = (
        LAYER_GROUPS["bottom"] + LAYER_GROUPS["middle"]
    )
    points["include_only_middle_top"] = (
        LAYER_GROUPS["middle"] + LAYER_GROUPS["top"]
    )
    points["include_only_bottom_top"] = (
        LAYER_GROUPS["bottom"] + LAYER_GROUPS["top"]
    )
