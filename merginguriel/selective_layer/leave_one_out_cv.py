...
class LeaveOneSourceOutCV:
    ...
    def run_all(
        self,
        project_root: str,
        holdout_filter: list[str] | None = None,
        ablation_point_filter: list[str] | None = None,
    ) -> pd.DataFrame:
        """Run all planned experiments.

        Args:
            project_root: Path to project root
            holdout_filter: If set, only run these holdout locales
            ablation_point_filter: If set, only run these ablation points
        """
        # Register plans
        ...
        # Apply filters
        def should_run(exp_id: int) -> bool:
            record = self.db.get(exp_id)
            if holdout_filter and record.holdout_locale not in holdout_filter:
                return False
            if ablation_point_filter and record.ablation_point not in ablation_point_filter:
                return False
            return True

        filtered_ids = [eid for eid in exp_ids if should_run(eid)]
        print(f"Running {len(filtered_ids)} experiments (after filters)")

        # Run baseline first (for each holdout)
        print("\n=== Running baselines first ===")
        baseline_ids = [
            eid for eid in filtered_ids
